...
<p align="center">
  <img alt="Version 0.0.2" src="https://img.shields.io/badge/version-0.0.2-0ea5e9" />
...
> [!IMPORTANT]
> `0.0.2` 是开发预览版本，适合本机试用和继续开发。目前尚未提供安装程序、代码签名或正式发布包。
...
它不是 Windows Shell 的完整替代品，也不是知识库。它是一套更专注、更稳定的项目文件工作流。

## 0.0.2 功能概览

- **项目级工作空间**：可以登记多个项目，但同一时间只激活一个；所有导航和文件操作都受当前项目根目录约束。
- **可靠的项目记忆**：项目列表使用带修订号的主记录、独立 Roaming 备份和上一版本快照；主记录被清空、损坏或丢失时自动恢复。
- **稳定的目录树**：惰性加载子目录，支持单击选择、双击展开/收起、自然排序和更大的树节点点击区域。
- **网格与列表浏览**：文件夹、PDF、Office、Markdown、代码、数据、压缩包等类型使用不同的语义图标；图片优先显示真实缩略图。
...
- **Windows 集成**：原生圆形 F 图标、任务栏身份、通知区域驻留、后台索引、左键双击恢复主窗口和完整退出菜单。
- **可选 MCP 适配器**：独立、默认关闭、只读；桌面应用本身不依赖 Codex 或云服务。

## 界面方向

当前界面采用 Focus Canvas：深色午夜/石墨背景、克制的青色交互状态、稳定的项目树和沉浸式预览层。

正式运行截图会在完成当前视觉回归后补充；本机设计草稿不会作为成品截图发布。
...
- 自动化核心测试：**19/19 通过**。
- 最新 `0.0.2` App Debug 构建：**0 个警告、0 个错误**。
- 项目根目录边界、注册表恢复、筛选、排序、重命名、复制/移动冲突、Markdown、代码预览、图片缩放/拖动和 SQLite 索引均有自动化覆盖。
- 批量剪贴板、真实回收站/撤销、完整视觉回归、可访问性、安装和发布就绪仍属于独立验证门槛。

构建成功、启动成功、交互测试、视觉 QA 和发布就绪在本项目中始终作为不同状态记录。
